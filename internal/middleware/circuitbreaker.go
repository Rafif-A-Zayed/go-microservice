...
import (
	...
	helper "user-management/internal/util"
)
...
func New(opts ...ExtraOptions) Circuitbreaker {
	...
	if opt.MaxFails == nil {
		opt.MaxFails = helper.ToPointer(uint64(5))
	}

	if opt.MaxConsecutiveFails == nil {
		opt.MaxConsecutiveFails = helper.ToPointer(uint64(5))
	}

	if opt.OpenInterval == nil {
		opt.OpenInterval = helper.ToPointer(5 * time.Second)
	}
